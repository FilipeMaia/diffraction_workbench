...
class slitItem(QtWidgets.QGraphicsRectItem):
    def __init__(self, index, mainWindow, x, y, w, h):
        ...
        self.setPos(x, y)
        self.min_y = np.linspace(
            0, mainWindow.sceneHeight-1, mainWindow.nrSources+1)[index]
        self.max_y = np.linspace(
            0, mainWindow.sceneHeight-1, mainWindow.nrSources+1)[index+1]-mainWindow.slitSize[index]
    ...
    def mouseReleaseEvent(self, event):
        ...
        self.mainWindow.toolpanel.positionSpin.setValue(
            self.y()+self.mainWindow.slitSize[self.index]/2.0)
...
class pointSourceItem(QtWidgets.QGraphicsEllipseItem):
    ...
    def mouseReleaseEvent(self, event):
        self.mainWindow.toolpanel.selectedSourceSpin.setValue(self.index+1)
        self.mainWindow.toolpanel.positionSpin.setValue(
            self.y()+self.mainWindow.pointSize/2)
        self.mainWindow.toolpanel.positionXSpin.setValue(
            self.x()+self.mainWindow.pointSize/2)


class pointSelector(QtWidgets.QGraphicsEllipseItem):
    def __init__(self, mainWindow):
        super(pointSelector, self).__init__(-mainWindow.pointSize/2.0, -
                                            mainWindow.pointSize/2.0, mainWindow.pointSize, mainWindow.pointSize)
        self.setFlag(QtWidgets.QGraphicsItem.ItemIsMovable)
        ...
        self.xLine = QtWidgets.QGraphicsLineItem(
            -mainWindow.nearSceneWidth, 0, mainWindow.nearSceneWidth, 0, self)
        self.xLine.setPen(QtGui.QPen(QtGui.QColor(
            "#3465a4"), 2, QtCore.Qt.DashLine, QtCore.Qt.RoundCap, QtCore.Qt.RoundJoin))
        self.xLine.setFlag(QtWidgets.QGraphicsItem.ItemStacksBehindParent)
        self.yLine = QtWidgets.QGraphicsLineItem(
            0, -mainWindow.sceneHeight, 0, mainWindow.sceneHeight, self)
        self.yLine.setPen(QtGui.QPen(QtGui.QColor(
            "#3465a4"), 2, QtCore.Qt.DashLine, QtCore.Qt.RoundCap, QtCore.Qt.RoundJoin))
...
class ArgandHand(QtWidgets.QGraphicsLineItem):
    ...
    def draw(self):
        ...
        self.setPos(self.farSceneWidth/2.0+self.originX,
                    self.farSceneWidth/2.0+self.originY)
        self.setPen(QtGui.QPen(QtGui.QColor(
            self.color), math.sqrt(self.scale)*4))
        self.arrowTop.setLine(0, 0, self.scale*-10, self.scale*-7)
        self.arrowTop.setPos(self.scale*(self.farSceneWidth/2.0-10), 0)
        self.arrowTop.setPen(QtGui.QPen(QtGui.QColor(self.color), math.sqrt(
            self.scale)*4, QtCore.Qt.SolidLine, QtCore.Qt.RoundCap, QtCore.Qt.RoundJoin))
        self.arrowBottom.setLine(0, 0, self.scale*-10, self.scale*7)
        self.arrowBottom.setPos(self.scale*(self.farSceneWidth/2.0-10), 0)
        self.arrowBottom.setPen(QtGui.QPen(QtGui.QColor(self.color), math.sqrt(
            self.scale)*4, QtCore.Qt.SolidLine, QtCore.Qt.RoundCap, QtCore.Qt.RoundJoin))
...
class Argand(QtWidgets.QGraphicsEllipseItem):
    def __init__(self, mainWindow):
        super(Argand, self).__init__(
            10, 10, mainWindow.farSceneWidth-20, mainWindow.farSceneWidth-20)
        ...
        line = QtWidgets.QGraphicsLineItem(
            farSceneWidth/2.0, 10, farSceneWidth/2.0, farSceneWidth-10, self)
        line.setPen(QtGui.QPen(mainWindow.white, 1, QtCore.Qt.DashLine,
                               QtCore.Qt.RoundCap, QtCore.Qt.RoundJoin))
        line = QtWidgets.QGraphicsLineItem(
            10, farSceneWidth/2.0, farSceneWidth-10, farSceneWidth/2.0, self)
        line.setPen(QtGui.QPen(mainWindow.white, 1, QtCore.Qt.DashLine,
                               QtCore.Qt.RoundCap, QtCore.Qt.RoundJoin))
        self.sources = []
    ...
    def addSource(self, i, angle):
        if(len(self.sources) <= i):
            self.sources.append(ArgandHand(
                self, self.farSceneWidth, 1.0/self.mainWindow.nrSources))
...
#		hbox.addStretch()
        ...
        self.animationSlider.valueChanged.connect(
            self.onAnimationSliderValueChanged)
    ...
    def initNearView(self):
        ...
        scene = QtWidgets.QGraphicsScene(
            0, 0, self.nearSceneWidth, self.sceneHeight)
    ...
    def initFarView(self):
        ...
        self.bottomArgand.setPos(
            0, self.animationBox.height()+self.farSceneWidth+48)
    ...
    def initToolPanel(self):
        ...
        toolpanel.nrSourcesSpin.valueChanged.connect(
            self.onNrSourcesSpinValueChanged)
        ...
        toolpanel.selectedSourceSpin.valueChanged.connect(
            self.onSelectedSourceSpinValueChanged)
        ...
        toolpanel.positionSpin.setValue(
            self.sourcePositions[toolpanel.selectedSourceSpin.value()-1])
        toolpanel.positionSpin.valueChanged.connect(
            self.onPositionSpinValueChanged)
        ...
        toolpanel.positionXSpin.setValue(
            self.sourceX[toolpanel.selectedSourceSpin.value()-1])
        toolpanel.positionXSpin.valueChanged.connect(
            self.onPositionXSpinValueChanged)
        ...
        toolpanel.phaseShiftDial.valueChanged.connect(
            self.onPhaseShiftDialValueChanged)
        toolpanel.phaseShiftDisplay = QtWidgets.QLCDNumber()
        toolpanel.phaseShiftDisplay.setNumDigits(3)
        toolpanel.phaseShiftDial.valueChanged.connect(
            toolpanel.phaseShiftDisplay.display)
        ...
        toolpanel.slitSizeSpin.setValue(
            self.slitSize[toolpanel.selectedSourceSpin.value()-1])
        toolpanel.slitSizeSpin.setVisible(False)
        toolpanel.slitSizeLabel.setVisible(False)
        toolpanel.slitSizeSpin.valueChanged.connect(
            self.onSlitSizeSpinValueChanged)
        ...
        toolpanel.wavelengthSpin.valueChanged.connect(
            self.onWavelengthSpinValueChanged)
        ...
        toolpanel.phaseDial.valueChanged.connect(
            toolpanel.phaseDisplay.display)
        ...
        toolpanel.simulateTimeCheck.toggled.connect(
            self.onSimulateTimeCheckToggled)
...
#		displayWhatGroup.setLayout(QtWidgets.QVBoxLayout())
        toolpanel.intensitiesRadio = QtWidgets.QRadioButton(
            "Averaged Intensities")
        toolpanel.intensitiesRadio.toggled.connect(
            self.onIntensitiesRadioToggled)
        toolpanel.amplitudesRadio = QtWidgets.QRadioButton(
            "Instantaneous E Field")
        toolpanel.amplitudesRadio.setChecked(True)
        toolpanel.amplitudesRadio.toggled.connect(
            self.onAmplitudesRadioToggled)
...
#		displayGroup.layout().addWidget(displayWhatGroup)
        ...
        toolpanel.farFieldRadio = QtWidgets.QRadioButton(
            "Far Field Intensities")
        toolpanel.pointCalculationRadio = QtWidgets.QRadioButton(
            "Animate Calculation ")
        toolpanel.pointCalculationRadio.setVisible(False)
        toolpanel.normalizeFarFieldCheck = QtWidgets.QCheckBox(
            "Normalize Far Field Intensities")
        toolpanel.pointCalculationRadio.setVisible(False)
...
#		displayGroup.layout().addWidget(displayWhatGroup)
        ...
        toolpanel.instantaneousRadio = QtWidgets.QRadioButton(
            "Instantaneous Values")
        toolpanel.integratedRadio = QtWidgets.QRadioButton(
            "Time Integrated Values")
        toolpanel.integratedRadio.toggled.connect(
            self.onTimeIntegratedRadioToggled)
...
#		vbox.addWidget(integrationGroup)
    ...
    def onNrSourcesSpinValueChanged(self, newValue):
        ...
        self.onSelectedSourceSpinValueChanged(
            self.toolpanel.selectedSourceSpin.value())
...
#			print "Instantaneous"
    ...
    def onPositionSpinValueChanged(self, newValue):
        if(self.sourcePositions[self.toolpanel.selectedSourceSpin.value()-1] == newValue):
            return
        self.sourcePositions[self.toolpanel.selectedSourceSpin.value(
        )-1] = newValue
        self.drawSources()
...
#			self.animationStep = 0
    ...
    def onPhaseShiftDialValueChanged(self, newValue):
        if(self.phaseShift[self.toolpanel.selectedSourceSpin.value()-1] != newValue):
            self.phaseShift[self.toolpanel.selectedSourceSpin.value() -
                            1] = newValue
            self.updateField()
    ...
    def drawSources(self):
        ...
        if(self.toolpanel.slitsRadio.isChecked()):
            wall = QtWidgets.QGraphicsRectItem(
                self.baseX-self.slitThickness/2+1, 0, self.slitThickness-2, self.sceneHeight)
        ...
        for i in range(0, self.nrSources):
            if(self.toolpanel.slitsRadio.isChecked()):
                source = slitItem(i, self, self.baseX-self.slitThickness/2,
                                  self.sourcePositions[i]-self.slitSize[i]/2, self.slitThickness, self.slitSize[i])
                ...
                source = pointSourceItem(
                    i, self, self.sourceX[i]-self.pointSize/2, self.sourcePositions[i]-self.pointSize/2, self.pointSize, self.pointSize)
    ...
    def calculateSlitFarField(self, pos, width, order):
        ...
        pos_boundaries = np.linspace(pos-width/2.0, pos+width/2.0, nsamples+1)
        pos_boundaries = (pos_boundaries[0:-1]+pos_boundaries[1:])/2.0
        farY = np.linspace(-self.sceneHeight*self.farX/self.nearSceneWidth/2,
                           self.sceneHeight*self.farX/self.nearSceneWidth/2, self.sceneHeight)
        for p in pos_boundaries:
            dist = np.sqrt((self.farX-self.baseX)**2.0 +
                           (farY-p)**2.0)/self.wavelength
    ...
    def calculateField(self):
        ...
        if(self.toolpanel.slitsRadio.isChecked()):
            order = 3
            for i in range(0, self.nrSources):
                self.E += self.calculateSlitField(
                    self.sourcePositions[i], self.slitSize[i], order)
                self.farE += self.calculateSlitFarField(
                    self.sourcePositions[i], self.slitSize[i], order)
            ...
            for i in range(0, self.nrSources):
                dist = np.sqrt(
                    (self.Ex-self.sourceX[i])**2.0+(y-self.sourcePositions[i])**2.0)/self.wavelength
                self.E += np.exp((-2.0j*math.pi*dist))*np.exp(-2.0j*math.pi*(
                    self.sourceX[i]-self.baseX)/self.wavelength)*np.exp(-1.0j*self.phaseShift[i]*math.pi/180.0)
                dist = np.sqrt(
                    (self.farX-self.sourceX[i])**2.0+(farY-self.sourcePositions[i])**2.0)/self.wavelength
                self.farE += np.exp((-2.0j*math.pi*dist))*np.exp(-2.0j*math.pi*(
                    self.sourceX[i]-self.baseX)/self.wavelength)*np.exp(-1.0j*self.phaseShift[i]*math.pi/180.0)
#		print time.time() - now
        self.drawField()
#		self.statusBar().showMessage('Done')
    ...
    def drawField(self):
        if(self.toolpanel.amplitudesRadio.isChecked()):
            colormap = np.real(
                self.E*np.exp(1.0j*math.pi*self.globalPhase/180.0))
            colormap -= np.min(colormap[:])
        else:
            colormap = np.abs(
                (self.E*np.exp(1.0j*math.pi*self.globalPhase/180.0)))**2
        c_max = np.max(colormap[:])
        if(c_max != 0):
            colormap *= 255.0/c_max
        colormap = colormap.astype(np.uint32)
        colormap = (255 << 24 | colormap[:, :] << 16 |
                    colormap[:, :] << 8 | colormap[:, :]).flatten()
        im = QtGui.QImage(
            colormap, self.E.shape[1], self.E.shape[0], QtGui.QImage.Format_RGB32)
    ...
    def plotFarField(self, fx):
        ...
        path.setPen(QtGui.QPen(self.skyBlue, 3, QtCore.Qt.SolidLine,
                               QtCore.Qt.RoundCap, QtCore.Qt.RoundJoin))
        self.plotObjects.append(path)
        self.farScene.addItem(path)
        path.setZValue(10)
        line = QtWidgets.QGraphicsLineItem(
            plot_start, 0, plot_start, self.sceneHeight)
        line.setPen(QtGui.QPen(self.orange, 1, QtCore.Qt.SolidLine,
                               QtCore.Qt.RoundCap, QtCore.Qt.RoundJoin))
...
#		self.farScene.addItem(path)
    ...
    def drawFarField(self):
        ...
        if(self.toolpanel.farFieldRadio.isChecked()):
            ...
            self.farScene.setSceneRect(
                0, 0, self.farSceneWidth, self.sceneHeight)
...
#				fx = np.abs((self.farE*np.exp(1.0j*math.pi*self.globalPhase/180.0)))**2
            fx = np.abs(
                (self.farE*np.exp(1.0j*math.pi*self.globalPhase/180.0)))**2
            offset = 0
            colormap = fx[:]-offset
            ...
            colormap = colormap.astype(np.uint32)
            colormap = (
                255 << 24 | colormap[:] << 16 | colormap[:] << 8 | colormap[:]).flatten()
            im = QtGui.QImage(
                colormap, 1, self.farE.shape[0], QtGui.QImage.Format_RGB32)
            pixmap = QtGui.QPixmap.fromImage(im)
            self.farFieldItem.setPixmap(pixmap.scaled(
                self.farSceneWidth, pixmap.height()))
            ...
            self.farScene.setSceneRect(
                0, self.sceneHeight*1.0/5.0, self.farSceneWidth, self.sceneHeight)
            ...
            self.animationBoxStep.setText(
                "Calculation Step: %d" % (self.animationStep))
            self.animationSlider.setValue(self.animationStep)
    ...
    def onAnimationSliderValueChanged(self, newValue, multiStep=False):
        ...
        for i in range(0, self.nrSources):
            if(self.animationStep <= step and newValue > step):
                line = QtWidgets.QGraphicsLineItem(
                    self.sourceX[i], self.sourcePositions[i], self.sourceX[i], self.sourcePositions[i])
                line.setZValue(10)
                grad = QtGui.QRadialGradient(QtCore.QPointF(
                    self.sourceX[i], self.sourcePositions[i]), self.wavelength)
    ...
    def onAnimationTimerTimeout(self):
        ...
        t = self.sender()
        t.step += 1
        self.sourceLines[i].setLine(self.sourceX[i], self.sourcePositions[i], self.sourceX[i]+float(
            t.step*t.deltaX)/t.maxStep, self.sourcePositions[i]+float(t.step*t.deltaY)/t.maxStep)
        self.topArgand.hand.setRotation(-self.phaseShift[i]-360*t.step*math.sqrt(t.deltaX**2+t.deltaY**2)/(
            t.maxStep*self.wavelength)-360*(self.sourceX[i]-self.baseX)/self.wavelength)
        if(t.step == t.maxStep):
            self.bottomArgand.addSource(i, -self.phaseShift[i]-360*t.step*math.sqrt(
                t.deltaX**2+t.deltaY**2)/(t.maxStep*self.wavelength))
    ...
    def onSceneClick(self, mouseEvent, scene):
        ...
        if(scene == self.farScene):
            v = (self.farFieldItem.pixmap().toImage().pixel(
                mouseEvent.scenePos().x(), mouseEvent.scenePos().y()) % 256)
            self.statusBar().showMessage('Intensity = %f' % (v*self.farScale))
        if(scene == self.nearScene):
            v = (self.fieldItem.pixmap().toImage().pixel(
                mouseEvent.scenePos().x(), mouseEvent.scenePos().y()) % 256)
